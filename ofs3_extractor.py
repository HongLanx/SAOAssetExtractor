...
class OFS3Tool:
    ...
    def unpack_ofs3(self, ofs3_file, destination_dir):
        ...
        with open(ofs3_file, 'rb+') as f:
            print(f"正在处理：{ofs3_file}")
            head = f.read(4)
            if head != b'OFS3':
                print(f"{ofs3_file}: Not a OFS3 File")
                return
            ...
            for i in range(n_files):
                if file_name_offset[i] >= 32:
                    file_name_start = file_name_offset[i]
            if file_name_start != 0:
                f.seek(file_name_start)
                file_name_size = os.path.getsize(ofs3_file) - file_name_offset[0]
                file_name_data = f.read(file_name_size).strip(b'\x00').split(b'\x00')
            ...
            for i in range(n_files):
                ...
                if type_ == 0x02 or type_ == 0x01:
                    if file_name_data and i < len(file_name_data) and file_name_data[i] != 0:
                        new_file_name = file_name_data[i].decode()
                    else:
                        new_file_name = ""
